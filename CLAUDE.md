...
- **brainstorming**: BEFORE new feature/component/non-trivial change. MANDATORY before code.
...
- **verification-before-completion**: before marking done. Verify end to end.
- **zoom-out**: stuck in bug loop or lost big picture.
- **requesting-code-review**: before merge or finishing feature branch.
...
- **critique**: review/evaluate UI. Scores with UX metrics, persona testing, anti-pattern detection.
...
Multi-turn via state: router returns `book_appointment` when `available_slots` non-empty or `selected_slot` set — no `interrupt_before` needed.
...
### Blocking I/O rule
HuggingFace embed + Chroma search: sync. Wrap in `run_in_executor`:
```python
